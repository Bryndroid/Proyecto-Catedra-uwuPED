...
namespace Proyecto_Catedra_PED.Categorias
{
    public partial class Tacos : Form
    {
        ...
        {
            InitializeComponent();
            c1.Visible = c2.Visible = c3.Visible = c4.Visible = c5.Visible
                  = false;
        ...
        {
            //TODOS LOS PRODUCTOS SIGUEN ESTA LOGICA
            //se inserta al final
            bool a = FormPrincipal.lista.InsertarF(label3.Text, 0, 0);
            //Luego se verifica si esta en la lista
            EstaEn(label3.Text, c1);
            if (a == false)
            {
                //Condicion por si no se ingresa valor
        ...
        {
            bool a = FormPrincipal.lista.InsertarF(label9.Text, 0, 0);
            EstaEn(label9.Text, c3);
            if (a == false)
            {
                MessageBox.Show("El producto ya se encuentra en el carrito",
                    "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        {
            bool a = FormPrincipal.lista.InsertarF(label6.Text, 0, 0);
            EstaEn(label6.Text, c5);
            if (a == false)
            {
                MessageBox.Show("El producto ya se encuentra en el carrito",
                    "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
